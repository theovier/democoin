    ...
    public synchronized boolean saveToDisc() {
        try {
            if (isValid()) {
                ...
                LOG.debug(String.format("saved blockchain, height: %d", getHeight()));
            } else {
    ...
    public static Blockchain loadFromDisc() {
        try {
            ...
            if (loadedBlockchain.isValid()) {
                LOG.info(String.format("blockchain with height %d successfully loaded.", loadedBlockchain.getHeight()));
